      ...
      <template #item="{ item }">
        <UTable v-if="item.key === 'members'" :rows="data?.members" :columns="columns">
          <template #role-data="{ row }">
            ...
            <UDropdown v-else :items="[roles.map(role => ({ label: role }))]" :ui="{ item: { base: 'capitalize' } }"
              @select="changeRole(row.email, $event)">
              <UButton variant="ghost" color="gray" :label="row.role" trailing-icon="i-heroicons-chevron-down-20-solid"
